      ...
      <div className="bg-gray-50 min-h-screen">
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          >
            <button
                onClick={handleBack}
                className="flex items-center bg-white text-blue-700 px-4 py-2 rounded-lg font-medium hover:bg-blue-50 transition duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
            >
              <svg
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-4 text-gray-700 text-sm mb-2">
                            <div className="flex items-center gap-1">
                              <FaMapMarkerAlt /> Route: <span className="font-semibold">{b.bus?.route?.from && b.bus?.route?.to ? `${b.bus.route.from} to ${b.bus.route.to}` : (b.route || 'N/A')}</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <FaCalendarAlt /> Date: <span className="font-semibold">{new Date(b.travelDate || b.date).toLocaleDateString()}</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <FaClock /> Departure: <span className="font-semibold">{b.bus?.schedule?.departureTime || b.departureTime || 'N/A'}</span>
                            </div>
                          </div>
                        </div>
                        <div className="flex flex-col gap-2 min-w-[180px]">
                          <div className="flex items-center gap-1">
                            <FaChair /> Seats: <span className="font-semibold">{Array.isArray(b.seats) ? b.seats.join(', ') : (b.seatNumber || 'N/A')}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <FaRupeeSign /> Price: <span className="font-semibold">NPR {b.totalPrice || b.amount || 0}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <FaCreditCard /> Payment: <span className="font-semibold">{b.payment ? 'Paid' : (b.paymentStatus || 'Pending')}</span>
                          </div>
                        </div>
                        <div className="flex flex-col gap-2 items-end min-w-[160px]">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${b.status === 'Confirmed' || b.status === 'confirmed' ? 'bg-blue-100 text-blue-700' : b.status === 'Cancelled' || b.status === 'Canceled' || b.status === 'cancelled' || b.status === 'canceled' ? 'bg-red-100 text-red-700' : b.status === 'Pending' || b.status === 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>
                      {b.status}
                    </span>
